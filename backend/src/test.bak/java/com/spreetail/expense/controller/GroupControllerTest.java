...
class GroupControllerTest {
    ...
    void testCreateGroupReturns201() {
        ...
        when(userService.getUserByEmail(anyString())).thenReturn(new UserResponse(1L, "test", "test@example.com", java.time.LocalDateTime.now()));
    ...
    void testDeleteGroupReturns200() {
        ...
        when(userService.getUserEmailFromToken(anyString())).thenReturn("test@example.com");
        when(userService.getUserByEmail(anyString())).thenReturn(new UserResponse(1L, "test", "test@example.com", java.time.LocalDateTime.now()));
        when(groupService.deleteGroup(anyLong(), anyLong())).thenReturn("Group deleted successfully");

        // Act
        ResponseEntity<?> response = groupController.deleteGroup(1L, "Bearer token");
